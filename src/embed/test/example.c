...
int main(void)
{
    ...
    printf("\n--- embed array range API ---\n");
    {
        ...
        if (!bun_array_set_range(ctx, array, 0, 4, write_values)) {
            fprintf(stderr, "[FAIL] bun_array_set_range(dense) failed: %s\n", bun_last_error(ctx, NULL));
        } else if (!bun_array_get_range(ctx, array, 1, 2, read_values)) {
            fprintf(stderr, "[FAIL] bun_array_get_range(dense) failed: %s\n", bun_last_error(ctx, NULL));
        } else if (bun_to_int32(read_values[0]) == 20 && bun_to_int32(read_values[1]) == 30) {
            printf("[PASS] bun_array_get_range/bun_array_set_range handle dense ranges\n");
        } else {
            fprintf(stderr, "[FAIL] dense range read returned [%d, %d] (expected [20, 30])\n",
...

    {
        ...
        if (!bun_array_set_range(ctx, array, 0, 2, initial_values)) {
            fprintf(stderr, "[FAIL] bun_array_set_range(initial) failed: %s\n", bun_last_error(ctx, NULL));
        } else if (bun_array_set_range(ctx, array, 1, 2, overflow_values)) {
            fprintf(stderr, "[FAIL] bun_array_set_range(no-growth) unexpectedly succeeded\n");
        } else {
            const char* err = bun_last_error(ctx, NULL);
            BunValue slot = bun_get_index(ctx, array, 1);
            if (err && bun_to_int32(slot) == 2) {
                printf("[PASS] bun_array_set_range rejects out-of-bounds writes before mutating\n");
            } else {
                fprintf(stderr, "[FAIL] bun_array_set_range(no-growth) left slot[1]=%d err=%s\n",
                    bun_to_int32(slot),
                    err ? err : "(null)");
            }
        }
...

    {
        ...
        if (bun_array_get_range(ctx, array, 3, 0, NULL) && bun_array_set_range(ctx, array, 99, 0, NULL)) {
            printf("[PASS] zero-count range operations succeed with NULL buffers\n");
        } else {
            fprintf(stderr, "[FAIL] zero-count range operations should succeed\n");
        }
    }

    {
        ...
        if (prototype_array == BUN_EXCEPTION) {
            fprintf(stderr, "[FAIL] creating prototype-backed array failed: %s\n", bun_last_error(ctx, NULL));
        } else {
            BunValue range_values[2] = { BUN_UNDEFINED, BUN_UNDEFINED };
            if (!bun_array_get_range(ctx, prototype_array, 0, 2, range_values)) {
                fprintf(stderr, "[FAIL] bun_array_get_range(prototype getter) failed: %s\n", bun_last_error(ctx, NULL));
            } else {
                ...
                if (bun_is_undefined(range_values[0]) && bun_to_int32(range_values[1]) == 99 && getter_hits != BUN_EXCEPTION && bun_to_int32(getter_hits) == 1) {
                    printf("[PASS] bun_array_get_range preserves holes and prototype getters\n");
                } else {
                    fprintf(stderr, "[FAIL] prototype getter range read returned unexpected values\n");
                }
            }
        }
    }

    {
        ...
        if (bun_array_get_range(ctx, non_array, 0, 1, range_values)) {
            fprintf(stderr, "[FAIL] bun_array_get_range(non-array) unexpectedly succeeded\n");
        } else if (bun_last_error(ctx, NULL)) {
            printf("[PASS] bun_array_get_range rejects non-array targets\n");
        } else {
            fprintf(stderr, "[FAIL] bun_array_get_range(non-array) did not set bun_last_error\n");
        }
    }

    {
        ...
        if (frozen == BUN_EXCEPTION) {
            fprintf(stderr, "[FAIL] creating frozen array failed: %s\n", bun_last_error(ctx, NULL));
        } else if (bun_array_set_range(ctx, frozen, 0, 1, &value)) {
            fprintf(stderr, "[FAIL] bun_array_set_range(frozen array) unexpectedly succeeded\n");
        } else if (bun_last_error(ctx, NULL)) {
            printf("[PASS] bun_array_set_range surfaces JS write failures\n");
        } else {
            fprintf(stderr, "[FAIL] bun_array_set_range(frozen array) did not set bun_last_error\n");
        }
    }
